...
class Validator implements \ArrayAccess {
     ...
     */
    public function __construct($rules = [], $options = [], $messages = [], $labels = null) {
    ...
    public static function expandFieldName($field) {
        ...
        return \array_merge(
            ...
            \preg_split('/\]\[/', \trim($matches[2], '[]')) // subfields
        );
    }
    ...
    public function validate(array $values, array $opt = []) {
        ...
        foreach ($this->rules as $field => $rules) {
            ...
            if ($ret !== true) {
                // hook disini=======================================================================
                $messages = [];
                foreach ($errors as $error=>$true) {
                    if (\is_array($true)) {
                        if (isset($this->messages[$error])) {
                            $label = (\is_object($this->labels)?$this->labels->$field:$field);
                            $msgv = \str_replace(':attrib', $label, $this->messages[$error]);
                            foreach ($true as $i=>$v) {
                                $msgv = \str_replace(':args['.$i.']', $v, $msgv);
                            }
                            $messages[] = $msgv;
                        } else {
                            $messages[] = $error.'['. \implode(',',$true) .']';
                        }
                    } else {
                        if (isset($this->messages[$error])) {
                            $label = (\is_object($this->labels)?$this->labels->$field:$field);
                            $messages[] = \str_replace(':attrib', $label, $this->messages[$error]);
                        } else {
                            $messages[] = $error;
                        }
                    }
    ...
    public function validateValue(&$value, array $rules, array &$errors = [], array $opt = []) {
        ...
        foreach ($rules as $rule => $param) {
            ...
            if ($rule === self::EACH) {
                ...
                $func = __NAMESPACE__ . '\\Rules\\Rules::' . $rule;
                if (!\is_callable($func)) {
                    $func = __NAMESPACE__ . '\\Rules\\Systems::' . $rule;
                    if (!\is_callable($func)) {
                        $func = '\\App\\Providers\\Rules::' . $rule;
                    }
                }
                if (\is_callable($func)) {
